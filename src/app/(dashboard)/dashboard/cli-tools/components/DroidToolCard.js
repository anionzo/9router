...
export default function DroidToolCard({
  ...
  const handleApplySettings = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        setMessage({ type: "success", text: t("cliTools.droid.messages.applySuccess") });
        checkDroidStatus();
      } else {
        setMessage({ type: "error", text: data.error || t("cliTools.droid.messages.applyFailed") });
      }
    } catch (error) {
      setMessage({ type: "error", text: error.message || t("cliTools.droid.messages.applyFailed") });
    } finally {
      setApplying(false);
    }
  };
  ...
  const handleResetSettings = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        setMessage({ type: "success", text: t("cliTools.droid.messages.resetSuccess") });
        setSelectedModel("");
        setSelectedApiKey("");
        checkDroidStatus();
      } else {
        setMessage({ type: "error", text: data.error || t("cliTools.droid.messages.resetFailed") });
      }
    } catch (error) {
      setMessage({ type: "error", text: error.message || t("cliTools.droid.messages.resetFailed") });
    } finally {
      setRestoring(false);
    }
  };
  ...
  const getManualConfigs = () => {
    const keyToUse = (selectedApiKey && selectedApiKey.trim()) 
      ? selectedApiKey 
      : (!cloudEnabled ? t("cliTools.common.apiKeyDefault") : t("cliTools.droid.placeholders.apiKey"));

    const settingsContent = {
      customModels: [
        {
          model: selectedModel || t("cliTools.droid.placeholders.model"),
          id: "custom:9Router-0",
          index: 0,
          baseUrl: getEffectiveBaseUrl(),
          apiKey: keyToUse,
          displayName: selectedModel || t("cliTools.droid.placeholders.model"),
  ...
  return (
    <Card padding="sm" className="overflow-hidden">
      <div className="flex items-center justify-between hover:cursor-pointer" onClick={onToggle}>
        <div className="flex items-center gap-3">
          <div className="size-8 flex items-center justify-center shrink-0">
            <Image src="/providers/droid.png" alt={t(tool.name)} width={32} height={32} className="size-8 object-contain rounded-lg" sizes="32px" onError={(e) => { e.target.style.display = "none"; }} />
          </div>
        ...
        <div className="mt-4 pt-4 border-t border-border flex flex-col gap-4">
          {checkingDroid && (
            <div className="flex items-center gap-2 text-text-muted">
              ...
              <span>{t("cliTools.droid.status.checking")}</span>
            </div>
          )}
          ...
          {!checkingDroid && droidStatus && !droidStatus.installed && (
            <div className="flex items-center gap-3 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
              ...
              <div className="flex-1">
                <p className="font-medium text-yellow-600 dark:text-yellow-400">{t("cliTools.droid.install.title")}</p>
                <p className="text-sm text-text-muted">{t("cliTools.droid.install.subtitle")}</p>
              </div>
            </div>
          )}
      ...
      <ModelSelectModal
        ...
        title={t("cliTools.droid.selectModelTitle")}
      />

      <ManualConfigModal
        isOpen={showManualConfigModal}
        onClose={() => setShowManualConfigModal(false)}
        title={t("cliTools.droid.manualConfigTitle")}
